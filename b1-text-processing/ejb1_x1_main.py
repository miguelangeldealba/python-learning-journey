...
def find_largest_word(text):
    ...
    for character in text:
        if is_space(character) or is_newline(character):
...
def find_size_largest_sentence(text, filter):
    ...
    for character in text:
        if is_newline(character):
            if filter in current_sentence and len(current_sentence) > max_size:
                max_size = len(current_sentence)
            current_sentence = ""
        else:
...
sprint("'a' es un palíndromo su resultado es:", is_palindrome_word("a"))
